...
public:
  HebiArmMover(const rclcpp::NodeOptions& options) : Node("hebi_bell_retreat", options) {
    ...
    trigger_sub_ = this->create_subscription<std_msgs::msg::Empty>(
        "/exec_retreat", 10, std::bind(&HebiArmMover::execute_move_callback, this, std::placeholders::_1), sub_options);
...
private:
  void execute_move_callback(const std_msgs::msg::Empty::SharedPtr) {
    RCLCPP_INFO(this->get_logger(), "===== Recieved /exec_retreat =====");
