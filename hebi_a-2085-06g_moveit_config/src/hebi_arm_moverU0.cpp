...
public:
  HebiArmMover(const rclcpp::NodeOptions& options) : Node("hebi_arm_moverU0", options) {
    ...
    trigger_sub_ = this->create_subscription<std_msgs::msg::Empty>(
        "/exec_moveU0", 10, std::bind(&HebiArmMover::execute_move_callback, this, std::placeholders::_1), sub_options);
