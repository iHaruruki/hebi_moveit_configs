...
public:
  HebiArmMover(const rclcpp::NodeOptions& options) : Node("hebi_arm_moverU1", options) {
    ...
    trigger_sub_ = this->create_subscription<std_msgs::msg::Empty>(
        "/exec_moveU1", 10, std::bind(&HebiArmMover::execute_move_callback, this, std::placeholders::_1), sub_options);
...
private:
  void execute_move_callback(const std_msgs::msg::Empty::SharedPtr) { 
    ...
    double offset_y = 0.03; 
    double offset_z = 0.15; 
