...
public:
        ...
        : Node("hebi_arm_mover4", options)
    {
        ...
        saved_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "/saved_pose", 10, std::bind(&HebiArmMover::savedPoseCallback, this, std::placeholders::_1),
            sub_options);

        trigger_sub_ = this->create_subscription<std_msgs::msg::Empty>(
            "/exec_move4", 10, std::bind(&HebiArmMover::execute_move_callback, this, std::placeholders::_1),
            sub_options);
...
private:
    constexpr static double lift_height = -0.02;
    constexpr static double y_axis_offset = -0.06;
    ...
    void execute_move_callback(const std_msgs::msg::Empty::SharedPtr)
    {
        ...
        target_pose.position.y += y_axis_offset;
